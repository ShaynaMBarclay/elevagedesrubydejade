...
export default function EditPedigree({ collection = "dogs" }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Prefill pedigree from existing data or fallback to dog/puppy info
      setPedigree({
        subject: {
  ...
  async function handleSave() {
    await updateDoc(doc(db, collection, id), { pedigree });
    alert("Pédigree mis à jour !");
    navigate(`/${collection === "dogs" ? "chiens" : "chiots"}/${id}/pedigree`);
  }

  return (
    <main className="edit-pedigree-page">
      <Link to={`/${collection === "dogs" ? "chiens" : "chiots"}/${id}/pedigree`} className="back-btn">← Retour</Link>
